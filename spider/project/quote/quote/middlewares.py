...
class QuoteDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class ProxyMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        if IP.status_code == 200:
            ...
            spider.logger.info("使用代理:" + request.meta['proxy'])
    ...
    def process_exception(self, request, exception, spider):
        ...
        if isinstance(exception, RetryMiddleware.EXCEPTIONS_TO_RETRY):
            spider.logger.error('代理连接超时!重新获取......')
            IP = requests.get(url=settings.get('PROXY_URL')).json()
            request.meta['proxy'] = "http://" + IP.get("ip")
            spider.logger.info("使用代理:" + request.meta['proxy'])
        return request
        # Must either:
